...
**Skills must not exceed 300 lines** to maintain focus and performance.

**Validation**:
```bash
# After creating skill, check size
wc -l .claude/skills/skill-name/skill.md

# If exceeds 300 lines:
# 1. Split into multiple focused skills
# 2. Move examples to separate documentation
# 3. Extract reference material to README.md
```

**Why 300 lines?**
- Skills should be single-purpose and focused
- Excessive size indicates multiple use cases (split into sub-skills)
- Smaller skills load faster and are easier to maintain

### Structure Requirements

**Required sections (in order)**:
1. YAML frontmatter (name, description, optional: allowed-tools, model)
2. Purpose (1-2 paragraphs)
3. When to Use (3+ specific triggers)
4. Process (step-by-step workflow)
5. Examples (1-2 detailed scenarios)
...
**Forbidden**:
- Extensive API documentation (→ create separate reference doc)
- Migration guides (→ create separate migration doc)
- Multiple distinct use cases (→ create separate skills)
...
1. **Check size**:
   ...
   if [ $lines -gt 300 ]; then
     echo "❌ Skill exceeds 300 lines ($lines). Refactor required."
     exit 1
   fi
   ```
...
3. **Check focus**:
   ...
   - No overlapping use cases with other skills
   - Examples support the main purpose

### Refactoring Oversized Skills

**If skill exceeds 300 lines**:

**Option 1: Split into Sub-Skills**
```
# Original (450 lines)
stripe-integration.md

# Split into (3 × 150 lines)
stripe-payments.md       # Payment processing
stripe-subscriptions.md  # Subscription management
stripe-webhooks.md       # Webhook handling
```

**Option 2: Extract Documentation**
```
# Skill (200 lines)
nextjs-app-router/skill.md

# Reference (separate file)
nextjs-app-router/README.md          # Installation, setup
nextjs-app-router/migration-guide.md # Migration from Pages Router
nextjs-app-router/api-reference.md   # Detailed API patterns
```

**Option 3: Consolidate Examples**
```
# Instead of 10 verbose examples (200 lines)
## Examples (50 lines total)
### Example 1: Common Use Case
[One comprehensive example covering 80% of uses]

### Example 2: Advanced Pattern
[One example for complex scenarios]

[Link to examples/ directory for more]
```

### Auto-Fix Recommendations

When creating skills, automatically suggest:

1. **If description < 100 chars**: Add trigger keywords and use cases
2. **If "When to Use" < 3 items**: Request more specific triggers
3. **If file size approaching 250 lines**: Suggest splitting before hitting limit
4. **If no examples**: Require at least 1 example before completion
5. **If multiple distinct purposes detected**: Suggest splitting into sub-skills

## Validation Checklist

Before finishing, verify:

- [ ] **Size check**: Skill is under 300 lines (current: ___) ⚠️ CRITICAL
...
- [ ] Single, focused purpose (no overlapping use cases)
